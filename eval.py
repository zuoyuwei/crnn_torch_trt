...
def get_img_label(label_path, images_path):
    ...
    img_label_list = []
    for line in lines[:-1]:
        this_img_path, this_img_label = line.split(' ', 1)
        _, filename= os.path.split(this_img_path)
        this_img_path = os.path.join(images_path, filename)
...
class crnn_eval:
    def __init__(self, imgp, labp, bs):
        ...
        self.crnn_handle = CRNNHandle.CRNNHandle(crnn_model_path, self.crnn_net, gpu_id=1.1)
        self.imgp = imgp
        self.labp = labp
        self.bs = bs
    ...
    def eval(self):
        ...
        if os.path.exists(val_label_path):
            train_img_list, train_label_list = get_img_label(self.labp, self.imgp)
            ...
            for j in range(batch_nums):
                ...
                for index, temp in enumerate(this_train_img_list):
                    image = Image.open(temp)
                    theres = self.crnn_handle.predict(image)
                    if this_train_label_list[index] == theres:
                        acc += 1
                    count += 1
                    print('acc : %f - %d , count: %d  ----%s >>> %s' % (acc/count, acc, count, this_train_label_list[index], theres))
        else:
            for root, dirs, files in os.walk(self.imgp):
                for file in files:
                    image = Image.open(os.path.join(root, file))
                    theres = self.crnn_handle.predict(image)

                    draw = ImageDraw.Draw(image)
                    fn1 = ImageFont.truetype("/home/work/PY/SynthText_Chinese_version/data/fonts/hei/hei.ttf", 32)
                    draw.text((0, 0), theres, fill=(255, 0, 255), font=fn1)
                    image.save(os.path.join(root + '/res/', file))
                    del draw


if __name__ == "__main__":
    val_image_path = '/home/work/PY/crnn_2/data/english_l'
    val_label_path = '/home/work/PY/crnn_2/data/english_l.txt'
    batch_size = 32
    crnneval = crnn_eval(val_image_path, val_label_path, batch_size)
    crnneval.eval()